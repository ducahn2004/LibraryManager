 ...
 */
public class LendingManagerService implements LendingManager {

  private static final Logger logger = LoggerFactory.getLogger(LendingManagerService.class);
  ...
  public boolean returnBookItem(BookItem bookItem, Member member, BookStatus status) {
    try {
      ...
      // Send notifications
      systemNotificationService.sendNotification(NotificationType.BOOK_RETURN_SUCCESS, bookLending.toString());
